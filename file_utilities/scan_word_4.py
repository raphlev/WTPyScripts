...
# Constant for processing paragraphs
PARAGRAPHS_TO_PROCESS = 20
    ...
    for file in files:
        if file.lower().endswith(('.doc', '.docx')):
            ...
            try:
                ...
                for idx in range(1, num_paragraphs + 1):
                    try:
                        para = doc.Paragraphs(idx)
                        # Get page number of paragraph
                        page_number = para.Range.Information(wdActiveEndPageNumber)
                        if page_number > PARAGRAPHS_TO_PROCESS:
                            break  # Stop processing
                        style = para.Style.NameLocal.lower()
                        text = para.Range.Text.strip()
                    except Exception as e:
                        # print(f"Error accessing paragraph {idx} in {file_path}: {e}")
