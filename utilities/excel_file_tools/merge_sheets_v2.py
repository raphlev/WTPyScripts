...
export_header = ['Excel','ID','PART_ID','EMPTY0','>','Mass','Package Type','Package Material','Package Class','Nb Pins','Package Pitch','Fire Resistance UL','Temperature Min','Temperature Max','Quality Lebel','MSL','Technology','Package Shape','Complexity','Type','Dielectric/Electrolyte','Ceramic Class','Part type/Configuration','Substrat Material']
...
for file in excel_files:
    ...
    for sn in (sheet_names):
        ...
        # Skip empty sheets
        # empty_df = pandas.read_excel(excel_file_path, sheet_name=sn, header=None, skiprows=2, nrows=1)
...
headers_df = pandas.DataFrame(columns=sorted(list(unique_headers), key=lambda x: custom_order.get(x, float('inf'))))
headers_df.to_excel('merged_data_distinct_headers.xlsx', index=False)
...
for file in excel_files:
    ...
    df_export = pandas.read_excel(excel_file_path, sheet_name='Export', header=None, skiprows=[0], dtype='str')

    if 'Excel' not in df_export.columns:
        df_export.insert(0, 'Excel', file)

    df_export.columns = export_header

    sheet_names = pandas.ExcelFile(excel_file_path).sheet_names

    # Reset merged df all merged df will be joind in join_df
    merged_df = headers_df
    ...
    for sn in (sheet_names):
        ...
        try:
            ...
            # Read each sheet into a DataFrame
            df = pandas.read_excel(excel_file_path, sheet_name=sn, header=0, skiprows=[1], dtype='str')

            if 'Sheet' not in df.columns:
                df.insert(0, 'Sheet', sn)
    ...
    # df_join = df_join._append(pandas.merge(df_export, merged_df, on='PART_ID', how='left'))
    df_join = pandas.concat([df_join, pandas.merge(df_export, merged_df, on='PART_ID', how='left')], ignore_index=True)

#print(list(df_join.columns))

print(f"Dropping unused columns and reordering...")
excluded_columns = ['ID_x', 'EMPTY0_x', '>_x']  # Names of columns to be excluded
selected_columns = [col for col in df_join.columns if col not in excluded_columns]

# Move the last header 'Sheet' to the second position
selected_columns.insert(1, selected_columns.pop(-1))
df_join = df_join[list(selected_columns)]

# Drop duplicated rows
# print(f"Dropping duplicated rows...")
# df_unique = df_join.drop_duplicates()

# Drop empty columns
print(f"Dropping empty columns...")
df_wo_empty_columns = df_join.dropna(axis=1, how='all')

# Create column which will mark duplicate PART_IDs and move near by PART_ID column
print(f"Creating column with duplicate flag...")
df_wo_empty_columns['IS_DUPLICATE'] = df_wo_empty_columns.duplicated(subset=['PART_ID'], keep=False)
selected_columns = list(df_wo_empty_columns.columns)
selected_columns.insert(3, selected_columns.pop(-1))
df_wo_empty_columns = df_wo_empty_columns[list(selected_columns)]

# Changing URL format
print(f"Changing url format...")
df_wo_empty_columns['P_DOC'] = df_wo_empty_columns['P_DOC'].str.extract(r'[\\/](Parts[\\/]\d+)[\\/]')
...
start_time = time.time()
df_wo_empty_columns.to_csv('merged_data_distinct_headers.csv', index=False)
end_time = time.time()
elapsed_time = round((end_time - start_time), 2)
print(f"Export to csv time taken   : {elapsed_time} seconds")

start_time = time.time()
df_wo_empty_columns.to_excel('merged_data_distinct_headers.xlsx', index=False)
end_time = time.time()
elapsed_time = round(((end_time - start_time) / 60), 2)
